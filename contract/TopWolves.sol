...
contract TopWolves is ERC721, Ownable, ReentrancyGuard {
    ...
    uint16 public secretFreeMintLimit = 500;
    uint16 public totalLimit = 5555;
    ...
    function verifyWhitelist(bytes32 leaf, bytes32[] memory _merkleProof) public view returns (bool){
		return MerkleProof.verify(_merkleProof, merkleRoot, leaf);
	}

    function setPresaleStart() external onlyOwner returns (uint256) {
        presaleStartTime = block.timestamp;
        mintStep = 1;
        return presaleStartTime;
    }

    function isWhiteListMember(bytes32[] memory _proof) external view returns (bool) {
        return verifyWhitelist(_leaf(msg.sender), _proof);
    }
    ...
    function mintPresale(uint8 _mintCount, bytes32[] memory _proof) external payable nonReentrant returns (uint256) {
        ...
        require((_mintCount <= MAX_PER_MINT)
                    && (block.timestamp <= presaleStartTime + presaleTime)
                    && (msg.value == (mintPriceDiscount * _mintCount))
                    && (adressPresaleCountMap[msg.sender] + _mintCount <= MAX_PER_MINT)
                );            
    ...
    function getAccountStatus(address account) external view returns (uint8[] memory) {
        ...
        status[0] = adressPresaleCountMap[msg.sender];
